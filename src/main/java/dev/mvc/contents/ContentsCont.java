...
//    request.setAttribute("cateVO", cateVO);
   ...
   * 목록 + 검색 + 페이징 지원
   * 검색하지 않는 경우
   * http://localhost:9091/contents/list_by_cateno.do?cateno=2&word=&now_page=1
   * 검색하는 경우
   * http://localhost:9091/contents/list_by_cateno.do?cateno=2&word=탐험&now_page=1
   * 
   * @param cateno
   * @param word
   * @param now_page
  ...
  public ModelAndView list_by_cateno(ContentsVO contentsVO) {
    ModelAndView mav = new ModelAndView();
     ...
     */
    String paging = contentsProc.pagingBox(contentsVO.getCateno(), contentsVO.getNow_page(), contentsVO.getWord(), "list_by_cateno.do", search_count);
    mav.addObject("paging", paging);
  
    // mav.addObject("now_page", now_page);
    
    mav.setViewName("/contents/list_by_cateno");  // /contents/list_by_cateno.jsp
  ...
  */
  ...
  public ModelAndView list_by_cateno_grid(ContentsVO contentsVO) {
     ModelAndView mav = new ModelAndView();
  ...
  * @return 페이징용으로 생성된 HTML/CSS tag 문자열
  */
   String paging = contentsProc.pagingBox(contentsVO.getCateno(), contentsVO.getNow_page(), contentsVO.getWord(), "list_by_cateno_grid.do", search_count);
   mav.addObject("paging", paging);
   
     // mav.addObject("now_page", now_page);
   
   mav.setViewName("/contents/list_by_cateno_grid");  // /contents/list_by_cateno_grid.jsp
  ...
  public ModelAndView read(int contentsno) { // int cateno = (int)request.getParameter("cateno");
    ModelAndView mav = new ModelAndView();
  ...
  public String delete_by_cateno(int cateno) {
    ...
    int cnt = this.contentsProc.delete_by_cateno(cateno);
